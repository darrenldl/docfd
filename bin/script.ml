type t = {
  commands : (int * string * Command.t) list;
}

exception Error_with_msg of string

let parse ~path : (t, string) result =
  try
    let lines =
      try
        CCIO.with_in path CCIO.read_lines_l
      with
      | Sys_error _ -> (
          raise (Error_with_msg (Fmt.str "failed to read script %s" (Filename.quote path)))
        )
    in
    let commands =
      lines
      |> CCList.foldi (fun acc i line ->
          let line_num_in_error_msg = i + 1 in
          if String_utils.line_is_blank_or_system_comment line then (
            acc
          ) else (
            match Command.of_string line with
            | None -> (
                raise (Error_with_msg
                         (Fmt.str "failed to parse command on line %d: %s"
                            line_num_in_error_msg line))
              )
            | Some command -> (
                (line_num_in_error_msg, line, command) :: acc
              )
          )
        ) []
      |> List.rev
    in
    Ok { commands }
  with
  | Error_with_msg msg -> Error msg

let run pool ~init_state (t : t) : (Session.Snapshot.t Dynarray.t, string) result =
  let snapshots = Dynarray.create () in
  try
    Dynarray.add_last
      snapshots
      (Session.Snapshot.make
         ~last_command:None
         init_state);
    let final_ver = List.length t.commands in
    CCList.foldi
      (fun state i (line_num_in_error_msg, original_line, command) ->
         match Session.run_command pool command state with
         | None -> (
             raise (Error_with_msg
                      (Fmt.str "failed to run command on line %d: %s"
                         line_num_in_error_msg original_line))
           )
         | Some (command, state) -> (
             let ver = i + 1 in
             let snapshot =
               Session.Snapshot.make
                 ~last_command:(Some command)
                 state
               |> (fun snapshot ->
                   if Session.should_keep_snapshot_state ~cur_ver:final_ver ver then (
                     snapshot
                   ) else (
                     Session.Snapshot.remove_state snapshot
                   )
                 )
             in
             Dynarray.add_last snapshots snapshot;
             state
           )

      )
      init_state
      t.commands
    |> ignore;
    Ok snapshots
  with
  | Error_with_msg msg -> Error msg
